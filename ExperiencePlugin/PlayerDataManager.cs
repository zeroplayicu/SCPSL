    ...
    public class PlayerDataManager
    {
        ...
        public PlayerDataManager(ExperiencePlugin plugin)
        {
            ...
            if (!Directory.Exists(_dataDirectory))
            {
                ...
                Logger.Info($"已创建数据目录: {_dataDirectory}");
            }

            // 加载所有玩家数据
            LoadAllData();

            Logger.Info($"已加载 {_playerDataCache.Count} 个玩家的数据");
        }
        ...
        public void LoadAllData()
        {
            ...
            {
                Logger.Error($"加载玩家数据失败: {ex.Message}");
        ...
        public void SaveAllData()
        {
            try
            {
                ...
                if (_plugin.Config.Debug)
                {
                    Logger.Debug($"已保存 {_playerDataCache.Count} 个玩家的数据到 {_dataFile}");
                }
            }
            catch (Exception ex)
            {
        ...
        public PlayerData GetOrCreatePlayerData(Player player)
        {
            ...
            Logger.Info($"新玩家数据已创建: {player.Nickname} (ID: {userId})");
        ...
        public bool AddExperience(Player player, int exp)
        {
            ...
            if (leveledUp)
            {
                Logger.Info($"玩家 {player.Nickname} 升级了！现在是 {data.Level} 级");
            }
